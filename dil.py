...
_METINLER = {
    ...
    'ruya_aciklama': {'tr': 'Gördüğünüz rüyayı olabildiğince ayrıntılı yazın. FalımaBak sembolleri yorumlar (eğlence amaçlıdır).', 'en': 'Describe your dream in detail. FalımaBak interprets the symbols (for entertainment).', 'de': 'Beschreiben Sie Ihren Traum ausführlich. FalımaBak deutet die Symbole (Unterhaltung).', 'fr': 'Décrivez votre rêve en détail. FalımaBak interprète les symboles (divertissement).', 'es': 'Describe tu sueño con detalle. FalımaBak interpreta los símbolos (entretenimiento).', 'ar': 'صف حلمك بالتفصيل. FalımaBak يفسر الرموز (للترفيه).', 'ru': 'Опишите сон подробно. FalımaBak толкует символы (развлечение).', 'pt': 'Descreva seu sonho. FalımaBak interpreta os símbolos (entretenimento).', 'it': 'Descrivi il sogno. FalımaBak interpreta i simboli (intrattenimento).', 'hi': 'सपना विस्तार से लिखें। FalımaBak प्रतीकों की व्याख्या करता है।', 'id': 'Ceritakan mimpi Anda. FalımaBak menafsirkan simbol (hiburan).', 'zh': '详细描述梦境。FalımaBak 解读符号（娱乐用途）。'},
