...
_METINLER = {
    ...
    'exit_msg': {'tr': 'Uygulamadan çıkılsın mı?', 'en': 'Exit the app?', 'de': 'App beenden?', 'fr': 'Quitter l\'application ?', 'es': '¿Salir de la aplicación?', 'ar': 'هل تريد الخروج؟', 'ru': 'Выйти из приложения?', 'pt': 'Sair do aplicativo?', 'it': 'Uscire dall\'app?', 'hi': 'ऐप बंद करें?', 'id': 'Keluar dari aplikasi?', 'zh': '要退出应用吗？'},
    'exit_yes': {'tr': 'Çık', 'en': 'Exit', 'de': 'Ja', 'fr': 'Oui', 'es': 'Sí', 'ar': 'نعم', 'ru': 'Да', 'pt': 'Sim', 'it': 'Sì', 'hi': 'हाँ', 'id': 'Ya', 'zh': '退出'},
    'exit_no': {'tr': 'İptal', 'en': 'Cancel', 'de': 'Nein', 'fr': 'Non', 'es': 'No', 'ar': 'لا', 'ru': 'Нет', 'pt': 'Não', 'it': 'No', 'hi': 'नहीं', 'id': 'Batal', 'zh': '取消'},
    ...
    'limit_msg': {'tr': 'Bugün {tip} için {limit} ücretsiz hakkınız doldu. Ödüllü reklam izleyerek +1 hak kazanın (bugün {reklam_kalan}/{reklam_max} reklam hakkı).', 'en': 'Free {tip} limit ({limit}) reached. +1 per rewarded ad ({reklam_kalan}/{reklam_max} left today).', 'de': '{tip}-Limit ({limit}). +1 pro Werbung ({reklam_kalan}/{reklam_max}).', 'fr': 'Limite {tip} ({limit}). +1 par pub ({reklam_kalan}/{reklam_max}).', 'es': 'Límite {tip} ({limit}). +1 por anuncio ({reklam_kalan}/{reklam_max}).', 'ar': 'حد {tip} ({limit}). +1 لكل إعلان ({reklam_kalan}/{reklam_max}).', 'ru': 'Лимит {tip} ({limit}). +1 за рекламу ({reklam_kalan}/{reklam_max}).', 'pt': 'Limite {tip} ({limit}). +1 por anúncio ({reklam_kalan}/{reklam_max}).', 'it': 'Limite {tip} ({limit}). +1 per annuncio ({reklam_kalan}/{reklam_max}).', 'hi': '{tip} ({limit})। +1/विज्ञापन ({reklam_kalan}/{reklam_max})।', 'id': 'Batas {tip} ({limit}). +1/iklan ({reklam_kalan}/{reklam_max}).', 'zh': '{tip}免费{limit}次已满。每次奖励广告+1（今日{reklam_kalan}/{reklam_max}）。'},
    'limit_reklam_doldu': {'tr': 'Bugün {tip} için tüm ödüllü reklam haklarınız doldu ({limit} ücretsiz + {reklam_max} reklam). Yarın yenilenir.', 'en': 'All rewarded ads used for {tip} today ({limit} free + {reklam_max} ads). Resets tomorrow.', 'de': 'Alle Werbungen für {tip} heute verbraucht ({limit}+{reklam_max}).', 'fr': 'Toutes les pubs {tip} utilisées ({limit}+{reklam_max}). Demain.', 'es': 'Todos los anuncios de {tip} usados ({limit}+{reklam_max}). Mañana.', 'ar': 'نفدت إعلانات {tip} ({limit}+{reklam_max}). غداً.', 'ru': 'Рекламы {tip} исчерпаны ({limit}+{reklam_max}). Завтра.', 'pt': 'Anúncios de {tip} esgotados ({limit}+{reklam_max}). Amanhã.', 'it': 'Annunci {tip} esauriti ({limit}+{reklam_max}). Domani.', 'hi': '{tip} विज्ञापन समाप्त ({limit}+{reklam_max})।', 'id': 'Iklan {tip} habis ({limit}+{reklam_max}). Besok.', 'zh': '{tip}奖励广告已用完（{limit}+{reklam_max}）。明日重置。'},
    ...
    'limit_ad_fail': {'tr': 'Reklam yüklenemedi. İnternet bağlantınızı kontrol edip tekrar deneyin.', 'en': 'Ad failed to load. Check connection and retry.', 'de': 'Werbung nicht geladen.', 'fr': 'Pub non chargée.', 'es': 'Anuncio no cargado.', 'ar': 'فشل تحميل الإعلان.', 'ru': 'Реклама не загрузилась.', 'pt': 'Anúncio não carregou.', 'it': 'Annuncio non caricato.', 'hi': 'विज्ञापन लोड नहीं हुआ।', 'id': 'Iklan gagal dimuat.', 'zh': '广告加载失败。'},
