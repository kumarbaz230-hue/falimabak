...
_METINLER = {
    ...
    'limit_msg': {'tr': 'Bugün {tip} için günlük {limit} hakkınız doldu (diğer fallar ayrı sayılır). Reklam izleyerek +1 {tip} hakkı kazanın. Yarın yenilenir.', 'en': 'Daily {tip} limit reached ({limit} per type). Watch an ad for +1 {tip} reading. Resets tomorrow.', 'de': '{tip}-Limit ({limit} pro Typ). Werbung für +1.', 'fr': 'Limite {tip} ({limit} par type). Pub pour +1.', 'es': 'Límite de {tip} ({limit} por tipo). Anuncio para +1.', 'ar': 'حد {tip} ({limit} لكل نوع). إعلان لـ +1.', 'ru': 'Лимит {tip} ({limit} на тип). Реклама за +1.', 'pt': 'Limite de {tip} ({limit} por tipo). Anúncio para +1.', 'it': 'Limite {tip} ({limit} per tipo). Pubblicità per +1.', 'hi': '{tip} सीमा ({limit} प्रति प्रकार). विज्ञापन से +1.', 'id': 'Batas {tip} ({limit} per jenis). Iklan untuk +1.', 'zh': '{tip}每日{limit}次（各类型独立）。看广告+1。'},
    'limit_watch_ad': {'tr': 'Reklam İzle', 'en': 'Watch Ad', 'de': 'Werbung', 'fr': 'Voir pub', 'es': 'Ver anuncio', 'ar': 'شاهد إعلان', 'ru': 'Смотреть рекламу', 'pt': 'Ver anúncio', 'it': 'Guarda annuncio', 'hi': 'विज्ञापन', 'id': 'Tonton iklan', 'zh': '看广告'},
